...
public async Task<IActionResult> Catalogo([FromQuery] CatalogoFiltroVM filtros, int page = 1)
{
    ...
    // SQLite no ordena por decimal: castea a double
    var items = await q.OrderBy(i => i.Ciudad)
                       .ThenBy(i => (double)i.Precio)
                       .Skip((page - 1) * PageSize)
                       .Take(PageSize)
                       .ToListAsync();

    var vm = new CatalogoVM { Filtros = filtros, Items = items, Page = page, TotalPages = totalPages, TotalCount = total };
    return View(vm);
}
           ...
           var visita = new Visita
{
    ...
    Notas       = vm.Notas ?? string.Empty   // ✅ usar 'Notas'
};
